...
public class Tele2  extends LinearOpMode {
    ...
    public void gp2_lb() {
        ...
        if (currentGamepad1LeftBumperState && !lastGamepad1LeftBumperState) {
            ...
            } else if(currentState == 2) { // currentState is 2 (or any other state that should go back to 1)
               deposit.setServoPosition(4, Constants.outtakeSampleReset[3]+0.05);
                currentState = 3;
            } else if (currentState == 3){
                new DepositResetCommand(deposit).schedule();
                new PDFLCommand(lifts, -10).schedule();
